...
<div class="flex flex-wrap">
    ...
    <h2><b>Daftar Petugas</b></h2>
            ...
            @if ($isModal)
                    @include('livewire.createpetugas')
            @endif
            ...
            <table class="bg-white text-gray-900 w-full shadow-none">
                <thead>
                    ...
                    <th class="bg-blue-700 text-white p-2">Jabatan Petugas</th>
                    <th class="bg-blue-700 text-white p-2">No. Telepon</th>
                    <th class="bg-blue-700 text-white p-2">Alamat</th>
                    <th class="bg-blue-700 text-white p-2">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                @forelse($petugas1 as $ptg)
                <tr class="bg-blue-100 text-blue-900">
                <!-- $loop->iteration -->
                <th class="p-2">{{$loop->iteration}}.</th>
                    <th class="p-2">{{$ptg->nama_petugas}}</th>
                    <th class="p-2">{{$ptg->jabatan_petugas}}</th>
                    <th class="p-2">{{$ptg->no_telp_petugas}}</th>
                    <th class="p-2">{{$ptg->alamat_petugas}}</th>
                    <th class="p-2">
                        <button wire:click="show({{$ptg->id_petugas}})" class="border" style="background-color:aqua; color:white">detail</button>
                        @if ($isModal1)
                            @include('livewire.detailpetugas')
                        @endif
                        <button wire:click="edit({{$ptg->id_petugas}})" class="border" style="background-color:green; color:white">edit</button>
                        <button wire:click="delete({{$ptg->id_petugas}})" class="badge badge-danger inline border" style="background-color:red; color:white">delete</button>
                    </th>
                </tr>
                @empty
                    <tr>
