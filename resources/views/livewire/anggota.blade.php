...
<div class="flex flex-wrap">
    ...
    <h2><b>Daftar Anggota</b></h2>
            ...
            @if ($isModal)
                    @include('livewire.createanggota')
            @endif
                ...
                <div id="modal" class="pacity-0 transform -translate-y-full scale-150  relative w-10/12 md:w-1/2 h-1/2 md:h-3/4 bg-white rounded shadow-lg transition-opacity transition-transform duration-300">
                    ...
                    <!-- body -->
                        <div class="w-full p-3">
                            ...
                            <br>
                            <input type="file" class="form-control @error('file') is invalid @enderror"  wire:model="file" required>
                            </div>
                        </div>
            ...
            <table class="bg-white text-gray-900 w-full shadow-none">
                ...
                <!-- $loop->iteration -->
                    ...
                    <th class="p-2">
                        <button wire:click="show({{$ag->id_anggota}})" class="border" style="background-color:aqua; color:white">detail</button>
                        @if ($isModal1)
                            @include('livewire.detailanggota')
                        @endif
                        <button wire:click="edit({{$ag->id_anggota}})" class="border" style="background-color:green; color:white">edit</button>
                        <button wire:click="delete({{$ag->id_anggota}})" class="badge badge-danger inline border" style="background-color:red; color:white">delete</button>
                    </th>
                </tr>
                @empty
                    <tr>
