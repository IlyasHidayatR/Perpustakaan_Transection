    ...
    <h2><b>Daftar Buku</b></h2>
            ...
            <table class="bg-white text-gray-900 w-full shadow-none">
                <thead>
                    ...
                    <th class="bg-blue-700 text-white p-2">Kode Buku</th>
                    <th class="bg-blue-700 text-white p-2">Judul Buku</th>
                    <th class="bg-blue-700 text-white p-2">Penulis</th>
                    <th class="bg-blue-700 text-white p-2">Penerbit</th>
                    <th class="bg-blue-700 text-white p-2">Tahun Terbit</th>
                    <th class="bg-blue-700 text-white p-2">Stok</th>
                ...
                <!-- $loop->iteration -->
                    ...
                    <th class="p-2">{{$bk->kode_buku}}</th>
                    <th class="p-2">{{$bk->judul_buku}}</th>
                    <th class="p-2">{{$bk->penulis_buku}}</th>
                    <th class="p-2">{{$bk->penerbit_buku}}</th>
                    <th class="p-2">{{$bk->tahun_penerbit}}</th>
                    <th class="p-2">{{$bk->stok}}</th>
                    <th class="p-2">
                        <button wire:click="show({{$bk->id_buku}})" class="border" style="background-color:aqua; color:white">detail</button>
                        @if ($isModal1)
                            @include('livewire.detailbuku')
                        @endif
                        <button wire:click="edit({{$bk->id_buku}})" class="border" style="background-color:green; color:white">edit</button>
                        <button wire:click="delete({{$bk->id_buku}})" class="badge badge-danger inline border" style="background-color:red; color:white">delete</button>
                    </th>
                </tr>
                @empty
                    <tr>
